package org.codefiesta.jbeanproxy.boundbean;
...
import org.codefiesta.jbeanproxy.util.BeanUtil;
import org.codefiesta.jbeanproxy.generic.WrapperProxyFactory;
...
public class BoundBeanFactory extends WrapperProxyFactory {
	...
	public void adjustProxyClass(CtClass proxyClass) throws Exception {
		...
		proxyClass.addField(changes, " new java.beans.PropertyChangeSupport(this)");

		CtMethod pcsGetter = CtNewMethod.getter("getPropertyChangeSupport", changes);
		proxyClass.addMethod(pcsGetter);

		CtClass boundBeanWrapperProxyIface = pool.get(BoundBean.class.getCanonicalName());
	...
	private void alterSetter(CtMethod setter) throws Exception {
		...
		setter.insertAfter("changes.firePropertyChange(\"" + property + "\", oldValue, $1);");
	}
