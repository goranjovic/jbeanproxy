...
public abstract class WrapperProxyFactory {
	...
	public <T> T createWrapperProxy(Object originalObject, Class<T> iface) {
		...
		try {
			...
			if (proxyClass == null) {
				...
				CtField origField = new CtField(ctIface, "original", proxyCc);
				proxyCc.addField(origField);

				CtClass[] parameters = new CtClass[] { ctBaseType };
				CtClass[] exceptions = new CtClass[] {};

				CtMethod putOriginalMethod = CtNewMethod.make(CtClass.voidType, "putOriginal",
						parameters, exceptions, "$0.original=$1;", proxyCc);
				proxyCc.addMethod(putOriginalMethod);

				CtMethod retrieveOriginalMethod = CtNewMethod.make(ctBaseType, "retrieveOriginal",
						new CtClass[] {}, exceptions, "return $0.original;", proxyCc);
				proxyCc.addMethod(retrieveOriginalMethod);

				CtConstructor defaultConstructor = CtNewConstructor.defaultConstructor(proxyCc);
